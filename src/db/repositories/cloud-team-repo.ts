...
/**
 * Return the IDs of teams where the given phone appears as a player.
 * Used on login so a player can see and access their team without being the owner.
 */
export async function fetchTeamIdsByPlayerPhone(phone: string): Promise<string[]> {
  if (!isCloudEnabled || !supabase) return [];
  try {
    const { data, error } = await supabase
      .from('cloud_players')
