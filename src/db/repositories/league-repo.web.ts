...
export async function createLeague(name: string, shortName: string): Promise<League> {
  const now = Date.now();
  const league: League = { id: uuidv4(), name, shortName, teamIds: [], createdAt: now, updatedAt: now };
...
export async function updateFixtureResult(
  ...
  setFixtures(getFixtures().map(f =>
    f.id === id
      ? { ...f, status: 'completed' as LeagueFixtureStatus, result, winnerTeamId, team1Score, team2Score, updatedAt: Date.now() }
      : f
  ));
}
