...
- `src/services/purchases.ts` — RevenueCat in-app purchase integration (`react-native-purchases@10.x` — **do not downgrade below v10**). `configurePurchases()` is called once on app startup (not on purchase); RC initializes `SKPaymentQueue` delegate immediately to catch pending transactions — this is why RC crashes **at launch**, not when the user taps Upgrade. On iOS 26 + New Architecture, the TurboModule bridge dispatches void methods on a background thread (ignoring `methodQueue = dispatch_get_main_queue()`), causing `SKPaymentQueue.delegate` assignment to throw an NSException → SIGABRT. The fix is in `patches/react-native-purchases+10.0.1.patch` (see below). `getCurrentPlan()` returns `'free' | 'pro' | 'league'`. `loginPurchasesUser(phone)` / `logoutPurchasesUser()` tie the RC anonymous user to the phone number on auth state change. Env vars: `EXPO_PUBLIC_RC_API_KEY_IOS`, `EXPO_PUBLIC_RC_API_KEY_ANDROID`. IAP product IDs in App Store Connect use a `_v2` suffix (`inningsly_pro_monthly_v2` etc.) because the original IDs were deleted and Apple blocks reuse; Android Play Console uses the originals without `_v2`. The `startsWith` matching in `upgrade.tsx` handles both automatically.
- `patches/react-native-purchases+10.0.1.patch` — **do not delete**. Patches `RNPurchases.m` (`setupPurchases`) to wrap `[RCPurchases configureWithAPIKey:...]` in a `dispatch_async(dispatch_get_main_queue(), ...)` block when not already on the main thread. Applied automatically by `npm postinstall` → `patch-package` on every `npm install`, including EAS builds. If RC is upgraded, re-run `npx patch-package react-native-purchases` after manually applying the same change to the new version, then strip any unrelated Android diffs from the generated patch file.
...
- **Do not** delete or skip `patches/react-native-purchases+10.0.1.patch` — without it, `setupPurchases` runs on a background thread on iOS 26 New Arch, causing `SKPaymentQueue.delegate` to throw → `std::terminate` → SIGABRT on every app launch.
