...
**iOS source build (required for iOS 26 / Xcode 17 and for the RN patch)** — `ios/Podfile.properties.json` sets `"ios.buildReactNativeFromSource": "true"` so React Native is compiled from source instead of using the pre-built CI binaries. Two reasons this is mandatory: (1) pre-built binaries have hardcoded paths incompatible with iOS 26; (2) `patches/react-native+0.83.4.patch` only applies when RN is compiled from source — it has no effect on pre-built binaries. If `expo prebuild --clean` regenerates `ios/Podfile.properties.json` and drops this key, restore it and re-stage with `git add -f ios/Podfile.properties.json` (the `/ios` gitignore rule blocks normal staging). The `post_install` hook in `ios/Podfile` sets `CLANG_CXX_LANGUAGE_STANDARD = 'c++20'` for all pod targets (required because RN bridging headers use C++20 `requires` concept syntax). After any `pod install` you may see `Removing React-Core-prebuilt / Removing ReactNativeDependencies` — this is expected and correct. When running from Xcode directly (not `npx expo run:ios`), Metro must already be running; launch `npx expo start` first. If `npx expo prebuild --clean` fails with `ENOTEMPTY`, run `rm -rf ios android` first then `npx expo prebuild`.
...
- **Do not** delete or skip `patches/react-native+0.83.4.patch` — without it, any NSException in any native module's async void method calls `convertNSExceptionToJSError(runtime, ...)` from a background thread → Hermes data race → SIGSEGV. Also requires `ios/Podfile.properties.json` `"ios.buildReactNativeFromSource": "true"`; if that key is missing (e.g. overwritten by `expo prebuild --clean`), restore it and re-add `ios/Podfile.properties.json` with `git add -f` (the `/ios` gitignore rule blocks normal staging).
